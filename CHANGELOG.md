...
- Spotify Connect (Phase 3A) über [`go-librespot`](https://github.com/devgianlu/go-librespot) als eigenen `mupibox-spotify.service` ergänzt: Gerät erscheint per Zeroconf in der offiziellen Spotify-App (kein OAuth, kein Client Secret für die Kernwiedergabe nötig), Wiedergabe über dasselbe konfigurierte ALSA/dmix-Gerät wie lokale Medien und TTS, Steuerung über `internal/providers/spotify` und `GET/POST /api/spotify/...` sowohl aus der Spotify-App als auch direkt am nativen Touchdisplay (Play/Pause/Next/Previous/Volume). Audio-Arbitration verhindert gleichzeitige Wiedergabe von lokalen Medien, Spotify und TTS in allen drei Richtungen. Auf echter Pi-4/MuPiHAT-V3.1-Hardware verifiziert, inklusive Neustart mit automatischer Wiederanmeldung. Web-API-Zugriff auf Bibliothek/Playlists/Suche (Phase 3B) folgt separat.
