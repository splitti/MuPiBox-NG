...
function renderWifiAdapters(){
 ...
 wifiAdapters.forEach(adapter=>{
  ...
  const power=document.createElement('button');power.type='button';power.textContent=adapter.active?t('wifi_power_off','Adapter ausschalten'):t('wifi_power_on','Adapter einschalten');power.onclick=()=>setWifiAdapterState(adapter,!adapter.active);
  ...
  row.append(enabled,primary,power,details);root.append(row)
 });
