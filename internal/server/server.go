...
func(a *API) Handler()http.Handler{
 ...
 mux.HandleFunc("GET /api/connectivity/wifi/adapters",func(w http.ResponseWriter,r *http.Request){if a.Connectivity==nil{problem(w,503,fmt.Errorf("connectivity manager unavailable"));return};adapters,err:=a.Connectivity.ListWiFiAdapters();if err!=nil{problem(w,503,err);return};jsonResponse(w,200,map[string]any{"adapters":adapters})})
 mux.HandleFunc("GET /api/connectivity/wifi",func(w http.ResponseWriter,r *http.Request){if a.Connectivity==nil{problem(w,503,fmt.Errorf("connectivity manager unavailable"));return};iface:=strings.TrimSpace(r.URL.Query().Get("interface"));var networks []connectivity.WiFiNetwork;var err error;if iface==""||iface=="all"{networks,err=a.Connectivity.ScanWiFi(r.Context())}else{networks,err=a.Connectivity.ScanWiFiOn(r.Context(),iface)};if err!=nil{problem(w,503,err);return};jsonResponse(w,200,map[string]any{"networks":networks})})
