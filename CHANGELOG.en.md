...
- Added Spotify Connect (Phase 3A) through [`go-librespot`](https://github.com/devgianlu/go-librespot) as its own `mupibox-spotify.service`: the device appears in the official Spotify app via Zeroconf (no OAuth, no client secret needed for core playback), plays through the same configured ALSA/dmix device as local media and TTS, and is controlled through `internal/providers/spotify` and `GET/POST /api/spotify/...` from both the Spotify app and directly on the native touchscreen (play/pause/next/previous/volume). Audio arbitration prevents local media, Spotify and TTS from playing over each other in all three directions. Verified on real Pi 4/MuPiHAT V3.1 hardware, including restart with automatic reconnection. Web API access to library/playlists/search (Phase 3B) follows separately.
